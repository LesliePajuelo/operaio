...
function Proxy(config) {
  ...
  this.blacklistStatus = config.blacklistStatus;
  this.blacklistMethod = config.blacklistMethod;
...
Proxy.prototype._doLaunch = function(cb) {
  ...
  this.java = spawn('java', ['-Dbmp.allowNativeDnsFallback=true', '-jar', jarPath, '-port', this.proxyPort]);
  var java = this.java;
...
Proxy.prototype.openProxy = function(cb) {
  ...
  if (this.limit) {
    proxySetupTasks.push(function(callback) {
      p.setLimit(self.port, self.limit, callback);
    });
  }
  if (this.blacklist) {
    proxySetupTasks.push(function(callback) {
      p.addURLBlackList(self.port, {
        'regex': self.blacklist,
        'status': self.blacklistStatus || 404,
        'method': self.blacklistMethod || ''
    }, callback);
    });
  }
  proxySetupTasks.push(function(callback) {
...
Proxy.prototype.saveHar = function(filename, result, cb) {
  this.log.info('Storing ' + filename);
  this.proxy.getHAR(this.port, function(err, har) {
    ...
    theHar.log.creator = {
      name: 'Browsertime',
      version: version,
      comment: 'Created using BrowserMob Proxy'
    };

    // BrowserMobProxy will take the name and version from the UserAgent
    // but we still want to know what the real browser is
    theHar.log.browser = {
